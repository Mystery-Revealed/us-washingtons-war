...
// download the PDF → End Session (deletes everything from memory). Washington's
// War is single-role solo (no pick), so students start on their own as soon as
// they're approved — no pairing, and one class group instead of a per-pick split.
...
const GROUP = 'class';
const TEKS = 'TEKS 8.4B, 8.4C, 8.22A, 8.31B';
  ...
  if (phase === 'gate') {
    return (
      <div className="app teacher-app">
        ...
        <p className="muted">Washington's War: Keep the Army Alive · {TEKS}</p>
  ...
  return (
    <div className="app teacher-app">
      <header className="row">
        <div>
          <h1>Teacher Command Center</h1>
          <p className="muted">Washington's War · session-only data · download the PDF before you end</p>
        </div>
...
function downloadPdf(roster, cls) {
  ...
  doc.setFontSize(16);
  doc.text("Washington's War: Keep the Army Alive — Session Report", 14, 16);
  doc.setFontSize(10);
  doc.setTextColor(90);
  ...
  doc.text('8th Grade U.S. History · Washington\'s War: Keep the Army Alive · TEKS 8.4B, 8.4C, 8.22A, 8.31B', 14, doc.internal.pageSize.getHeight() - 8);

  doc.save(`washingtons-war_${roster.joinCode}_${date}.pdf`);
}
